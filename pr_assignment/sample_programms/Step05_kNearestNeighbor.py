...
# CalcDistance
# 今回は L1 距離を算出
def CalcDistance_L1(target, template):
    buf = np.fabs(target - template)
    return np.sum(buf)


def CalcDistance_L2(target, template):
    return np.linalg.norm(target - template)
...
def ReturnMatchLabel(target, templates, labels, K):
    ...
    for i in range(0, TrainingSampleNum):
        dis = CalcDistance_L2(target, templates[i, :, :])
        label = labels[i]
        for k in range(0, K):
            if min_dis[k] > dis:
...
def step05_run():
    ...
    toc = time.time()
    print("Original method took %f seconds" % (toc - tic))
