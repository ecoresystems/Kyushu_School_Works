...
# 
combination = [[2, 64, 512], [2, 32, 1024], [4, 16, 512], [4, 32, 256], [8, 16, 128], [16, 8, 64], [16, 4, 128],
               [32, 4, 32], [8, 8, 256]]
    ...
    for item in combination:
        ...
        for l1_assoc in associativity:
            ...
            for l2_assoc in associativity:
                try:
                    with open("result/core%d_cache%dKB_asso%d.txt" % (cores, l2_size_pc * cores, l2_assoc), 'r') as f:
                        for line in f:
                            line = line.strip()
                            if "Access time" in line:
                ...
                aux = pd.DataFrame(np.array([[cores, clock, l1d_size, l1_assoc, l2_size, l2_assoc, l2_latency]]),
...
def cmd_assembler(cores, clock, l1d_size, l1d_assoc, l2_size, l2_assoc, l2_latency):
    cmd_dict = dict()
    cmd_dict['fmm'] = ('./splash2/fmm/FMM < ./splash2/inputs/input.2048.p%d' % cores)
